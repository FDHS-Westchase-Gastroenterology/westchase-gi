...
const base64PrivateKeyPemMarkers = ["", "RSA", "EC", "OPENSSH"].map((keyType) =>
  Buffer.from(["-----BEGIN", keyType, "PRIVATE KEY-----"].filter(Boolean).join(" "), "utf8")
    .toString("base64")
    .replace(/=+$/, ""),
);
...
function readServiceKeyPrefix() {
  const envValues = parseDotEnv(readFileSync(resolve(repoRoot, ".env.local"), "utf8"));
...
function main() {
  ...
  const allPaths = runGit(["log", "--all", "--name-only", "--format=%x1e%H"]);
  ...
  if (findings.size > 0) {
    console.error(`Secret-history verification failed with ${findings.size} redacted finding(s):`);
    for (const finding of findings.values()) {
      console.error(`- ${finding.label}: ${finding.file} @ ${shortCommit(finding.commit)}`);
    }
    process.exitCode = 1;
    return;
  }
