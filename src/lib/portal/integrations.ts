...
export type GitHubRepositoryStatus =
  ...
  | {
      ...
      account: string;
      repository: string;
      installationScope: "Selected repositories" | "All repositories — narrow in GitHub";
    };
...
export async function getGitHubRepositoryStatus(): Promise<GitHubRepositoryStatus> {
  ...
  try {
    ...
    const repository = repositoriesResponse.repositories.find(
      (candidate) =>
        isRecord(candidate) && candidate.full_name === CANONICAL_REPOSITORY,
    );
    if (!isRecord(repository) || repository.full_name !== CANONICAL_REPOSITORY) {
