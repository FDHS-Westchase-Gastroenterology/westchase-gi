...
import { timeWithinDay } from "./record-card-time";
...
/* Wall-clock times live in ./record-card-time; the model only asks whether
...
/** The one thing a disabled Save is still waiting for — a call-again's
   day, a booking's day, or its time — or null while the draft is blank
   (nothing asked yet) or already saveable. */
export function saveHintFor(draft: Readonly<CardDraft>, today: string): string | null {
  if (draft.answer === null || canSave(draft, today)) return null;
  if (needsTime(draft.answer)) {
    return withinHorizon(draft.day, today, dayHorizon(draft.answer))
