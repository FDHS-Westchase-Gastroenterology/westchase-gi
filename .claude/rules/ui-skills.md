...
- **shadcn** (model-invoked only; it has no slash command): before adding, fixing, styling, or composing any component, and before any `shadcn` CLI command. Pair it with AGENTS.md "shadcn/ui": reuse existing recipes, inspect registry provenance, and explain custom adaptations. Claude Design approval is not required. `src/components/stock/` is bundle input rather than approved design, and every CLI operation that touches CSS runs the `globals.css` reconciliation procedure. `init`, `apply`, and anything with `--preset` never run here without that review.
- **apple-design**: before implementing a sheet, drawer, drag, swipe, or gesture, and before changing how a popover, dialog, or sheet enters, exits, flips, or is dismissed. It supplies the physics (velocity handoff, momentum projection, rubber-banding, clean interruption); the registry supplies the values. Before proposing the approach for any interaction, component, or flow, read its `APPLE-DOCS.md`: Apple has usually solved the situation already, and the plan mimics that pattern rather than inventing one; cite the pages the way design-system/overlays.md "Companion surfaces" does. Hover, press, and checked micro-transitions on buttons and rows do not need it.
...
AGENTS.md hard rules outrank the skills. DESIGN.md and `design-eng.md` outrank a skill's generic values. The skills bring method and judgment; the registry brings every number: the `--motion-*` tokens in `src/app/globals.css` and the presets in `src/lib/motion.ts` own curves, durations, and springs. Where a skill's values differ (its default spring, its duration tables, an easing it suggests), use the registry's. Discuss a temperament the registry lacks with Jason per DESIGN.md "Motion", then add it to the registry rather than using an inline value. Where a skill's output format differs from `design-eng.md` "Review Format", the repo's format wins and the skill's standards and escalation rules still apply.
