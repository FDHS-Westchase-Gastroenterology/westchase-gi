// forked from design-sync lib/dts.mjs - nest shadcn's flat named exports into component families (see mergeRepoFamilies at the bottom)
...
export function loadDts(typesRoot) {
  ...
  mergeRepoFamilies(compounds);
  return { project, entry, pkgDir, nonComponents, compounds };
}

/* ── FORK ADDITION ────────────────────────────────────────────────────────
 * Merge the file-derived component families from .design-sync/ds/compounds.json.
 *
 * Why: design-sync infers subcomponents from compound statics (`Card.Header`)
 * or namespace exports. shadcn/Base UI ship FLAT named exports — CardHeader is
 * its own top-level function — so nothing in the .d.ts relates it to Card, and
 * all 444 exports emit as sibling root cards.
 *
 * The relationship is real, it just lives in the repo's tier contract instead
 * of the type system: one registry item per file (src/components/stock/
 * README.md + MANIFEST.json). gen-entry.mjs reads that structure and writes
 * compounds.json; this merge hands it to partitionSubcomponents, which nests
 * the 316 subcomponents under their 50 parents.
 *
 * Additive only — anything the checker genuinely found wins.
 */
function mergeRepoFamilies(compounds) {
  const file = join(dirname(new URL(import.meta.url).pathname), '..', 'ds', 'compounds.json');
  if (!existsSync(file)) return;
  let repo;
  try {
    repo = JSON.parse(readFileSync(file, 'utf8'));
  } catch (e) {
    console.error(`[OVERRIDE] compounds.json unreadable (${e.message}) — falling back to flat cards`);
    return;
  }
  let added = 0;
  for (const [root, members] of Object.entries(repo)) {
    const have = compounds.get(root) ?? [];
    const merged = [...new Set([...have, ...members])];
    if (merged.length !== have.length) added++;
    compounds.set(root, merged);
  }
  console.error(`  [OVERRIDE] merged ${added} repo-derived component families from ds/compounds.json`);
}
