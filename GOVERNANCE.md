...
3. **Trusted merge controller:** every verified manifest-only npm update can enter the queue.
   ...
   exact-head checks suppressed by GitHub's token recursion rule; attests protected contexts only
   after the real check runs pass on that SHA; ignores only the coarse `unstable` state caused by
   redundant optional runs; and submits at most one normal, non-bypass merge. GitHub remains the
   final branch-policy authority. The next PR waits for post-merge CI, React Doctor, the matching
