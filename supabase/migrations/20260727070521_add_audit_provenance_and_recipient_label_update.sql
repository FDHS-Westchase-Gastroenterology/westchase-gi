alter table public.audit_log
  ...
  add constraint audit_log_source_valid
    check (source in ('staff', 'system', 'acceptance'));

create function public.portal_update_recipient_label(
  p_actor_email text,
  p_recipient_id uuid,
  p_label text
)
...
begin
  if p_label is not null
    and (
      v_label = ''
      or pg_catalog.char_length(v_label) > 120
    )
  then
    raise exception 'Labels must be 1-120 characters or null'
      using errcode = '22023';
  end if;
  ...
  if v_previous_label is not distinct from v_label then
    return false;
  end if;
  ...
  insert into public.audit_log (
    ...
    p_actor_email,
    'recipients.label_update',
    'notification_recipients',
    p_recipient_id,
    'staff',
    pg_catalog.gen_random_uuid(),
    pg_catalog.jsonb_build_object(
      'from', v_previous_label,
      'to', v_label
    )
  );
...
revoke execute on function public.portal_update_recipient_label(text, uuid, text)
from public, anon, authenticated;
grant execute on function public.portal_update_recipient_label(text, uuid, text)
to service_role;
