-- Record a contact fact and finish an open request in one transaction.
-- The neutral closure reason records the staff decision without inventing a
-- patient refusal. A missing call-again date remains invalid on the older
-- callback command; callers must choose this explicit completion command.

alter table public.requests
  drop constraint requests_closure_reason_valid,
  add constraint requests_closure_reason_valid check (
    closure_reason is null or closure_reason in ('not_actionable', 'wont_schedule', 'no_further_contact')
  );
alter table public.request_transitions
  ...
  add constraint request_transitions_command_check check (
    command in ('record_contact_attempt', 'record_contact_and_close', 'confirm_booking_handoff', 'close_request',
      'reopen_request', 'set_call_again', 'undo_latest_transition', 'classify_legacy_closure')
  ),
  drop constraint request_transitions_reason_code_check,
  add constraint request_transitions_reason_code_check check (
    reason_code is null or reason_code in ('reached_follow_up', 'reached', 'voicemail', 'no_answer',
      'not_actionable', 'wont_schedule', 'booked')
  );
...
begin
  ...
  -- Another copy of this save may have completed while this caller waited for
  -- the request lock. Replay it before comparing the now-advanced version.
  select * into r
  from public.request_command_receipts
  where request_id = p_request_id and idempotency_key = p_idempotency_key;
  if found then
  ...
  if cmd = 'undo_latest_transition' then
    ...
    if next_state not in ('new', 'contacted', 'booked', 'closed')
      ...
      or (next_state = 'closed' and (
        ...
        or (not next_legacy_review_required and (
          ...
          or next_closure_reason not in ('not_actionable', 'wont_schedule', 'no_further_contact')
        ))
      ))
    then
      return '{"ok":false,"code":"undo_unavailable"}'::jsonb;
    ...
    if cmd = 'record_contact_attempt' then
    ...
    elsif cmd = 'record_contact_and_close' then
      if v.status not in ('new', 'contacted')
        or next_state is distinct from 'closed'
        or next_reason_code is null
        or next_reason_code not in ('reached', 'voicemail', 'no_answer')
        or next_call_again_at is not null
        or next_booking_confirmed_at is not null
        or next_appointment_at is not null
        or next_closed_at is distinct from now_at
        or next_closure_reason is distinct from 'no_further_contact'
        or p_decision ->> 'legacyReviewRequired' is distinct from 'false'
      then
        return '{"ok":false,"code":"invalid_command"}'::jsonb;
      end if;
      ...
      if p_decision ->> 'appointmentAt' is not null
        or next_closure_reason = 'no_further_contact'
      then
        return '{"ok":false,"code":"invalid_command"}'::jsonb;
      end if;
    ...
    elsif cmd = 'classify_legacy_closure' and next_closure_reason = 'no_further_contact' then
      return '{"ok":false,"code":"invalid_command"}'::jsonb;
    ...
    next_closure_provenance := case
      ...
      when cmd in ('close_request', 'record_contact_and_close', 'classify_legacy_closure') then null
  ...
  if cmd = 'record_contact_and_close' then
    insert into public.request_events(request_id, type, status, meta, created_at)
    values (
      p_request_id, 'contact_completed', 'recorded',
      pg_catalog.jsonb_build_object(
        'outcome', next_reason_code,
        'closure_reason', next_closure_reason,
        'author_email', p_actor_email
