-- Restore the previous commands only before the new contact-completion
-- vocabulary has been used. Keep recorded patient work intact on rollback.
-- If this guard fails, keep the current schema and use a forward correction.
do $$
begin
  if exists (select 1 from public.requests where closure_reason='no_further_contact')
    or exists (select 1 from public.request_transitions where command='record_contact_and_close' or reason_code='reached')
    or exists (select 1 from public.request_events where type='contact_completed')
  then
    raise exception 'Recorded contact completions require a forward correction; rollback would discard their meaning'
      using errcode='55000';
  end if;
end;
$$;
...
alter table public.requests
  ...
  add constraint requests_closure_reason_valid check (
    closure_reason is null or closure_reason in ('not_actionable', 'wont_schedule')
  );
alter table public.request_transitions
  ...
  add constraint request_transitions_command_check check (
    command in ('record_contact_attempt', 'confirm_booking_handoff', 'close_request',
  ...
  add constraint request_transitions_reason_code_check check (
    reason_code is null or reason_code in ('reached_follow_up', 'voicemail', 'no_answer',
