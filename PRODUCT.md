...
- **Home** owns orientation, triage, handoff, and the next useful action: a triage view of the
  ordered shared work stack, the direct New-request print action, the next request to work, recent
  operational context, and routes into the jobs that need attention.
- **Appointments** owns the complete appointment-request lifecycle and working queue, retaining
  All, New, Contacted, Scheduled, and Closed as familiar views.
