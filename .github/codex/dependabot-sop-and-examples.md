...
- **Success criteria:** Every verified manifest-only npm update may enter the
  ...
  waits for verified Production and live health. Transient, stale, or generated
  lockfile failures retry automatically; concrete defects are rejected rather
  than delegated.
...
database-deployment agent. Codex remains read-only. `retry` asks Dependabot to
rebase, `repair` asks Dependabot to recreate its generated manifest/lockfile
change, and `reject` closes a concrete incompatible update. Source or migration
work is never improvised inside a secret-bearing dependency run.
...
7. Select the oldest exact-head-approved PR whose gates are green. A failing
   older PR does not stall a green sibling. Rebase a behind/conflicting branch,
   retry transient Codex failures up to three times, and recreate generated
   dependency files when Codex returns `repair`.
...
10. Retry an incomplete metadata fetch without exposing the PR to Codex. Close
    ...
    green PRs. Malformed model output is `retry`, never approval.
...
| 4 | Other runtime update (`next`, React, Resend, or Zod) | Eligible | Exact-head deterministic and semantic gates all pass |
...
| 10 | Codex times out or returns malformed JSON | Retry automatically | No model-output parsing can accidentally approve |
