...
schema-changing migration ships with a rollback sibling in `supabase/rollbacks/`. Supabase
replays the committed lineage into the PR's isolated Preview Branch; both its native
deployment check and the hosted integration suite must pass on the exact head. Merging does
not authorize Production promotion. When old and new application versions can overlap,
schema-first changes keep the deployed interface backward compatible until the new
application is live. Fictional seed fixtures live in `supabase/seed.sql` and
